...
def seed_database(db):
    ...
    # --- 3. 学部マスターデータの登録 ---
    university_id_map = {u.name: u.id for u in University.query.all()}
    for uni_name, fac_name in faculties_to_seed:
        if uni_name in university_id_map:
            uni_id = university_id_map[uni_name]
            if not Faculty.query.filter_by(university_id=uni_id, name=fac_name).first():
    ...
    for task_id, title, desc, yt, weeks, type in books_to_seed:
        if not Book.query.filter_by(task_id=task_id).first():
    ...
    # --- 6. ルートステップ登録 ---
    book_id_map = {b.task_id: b.id for b in Book.query.all()}
    route_id_map = {r.name: r.id for r in Route.query.all()}
    for route_name, task_id, step, level, cat, is_main in route_steps_human_readable:
        if route_name in route_id_map and task_id in book_id_map:
            ...
            if not RouteStep.query.filter_by(route_id=route_id, book_id=book_id, step_order=step).first():
    ...
    for subject_name, strategy_html in strategy_data.items():
        if subject_name in subject_id_map:
            ...
            if not SubjectStrategy.query.filter_by(subject_id=subject_id).first():
